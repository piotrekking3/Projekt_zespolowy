...
WMB.Marker = {
	...
	*/
	onAddSubmit: function() {
		if ($("#address").val().length < 4 || $("#address").val().length > 32) {
			...
			$.ajax({
				...
				success: function (response) {
					id_zgloszenia = response.id_zgloszenia;
			...
			post_data.append('id', id_zgloszenia);
			post_data.append('file', file);
			var xhr = new XMLHttpRequest();.
			xhr.open('POST', 'https://m.bariery.wroclaw.pl/api/zdjecia/post', true);
			xhr.onload = function(oEvent) {
				if (xhr.status == 200) {
					alert('Uploaded');
				} else {
					console.log('Image upload error: ' + xhr.status);
				}
			};
			xhr.send(post_data);
