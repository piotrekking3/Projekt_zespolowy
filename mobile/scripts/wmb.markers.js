...
// Przypisanie ID do poszczególnych kategorii
...
// Przypisanie ID do poszczególnych ikon
...
// Wczytywanie znaczników z REST API
$.ajax({
	...
	success: function(data) {
		$.each(data, function(i) {
			// Wczytujemy tylko nowe i nie rozwiązane zgłoszenia
			if (data[i].zgloszenia.id_statusu != 4 && data[i].zgloszenia.id_statusu != 3) { 
				L.marker([data[i].zgloszenia.wspolrzedne.x, data[i].zgloszenia.wspolrzedne.y], {icon: idToIcon(data[i].zgloszenia.id_typu)}).setOpacity(0.80).bindPopup("<b>" + data[i].zgloszenia.adres + "</b><br>"+ typ[data[i].zgloszenia.id_typu] + "<br><i>" + data[i].zgloszenia.opis + "</i> (<a href=\"#\">czytaj więcej</a>)").addTo(idToCategory(data[i].zgloszenia.id_typu));
			}
		});
	}
});